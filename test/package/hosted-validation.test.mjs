...
test('canonical comparison accepts four equal manifests and rejects drift', async () => {
	...
	try {
		...
		const canonical = {
			package: { name: '@stratejya/operon-cli', version: '1.1.1' },
			tarball: {
		...
		for (const name of ['canonical-ubuntu-24.04', 'canonical-macos-14', 'canonical-windows-2022', 'canonical-windows-2025']) {
			...
			await writeFile(path.join(directory, 'operon-cli-1.1.1.tgz'), fixture.tarball);
		...
		const tamperedTarball = path.join(input, 'canonical-windows-2025', 'operon-cli-1.1.1.tgz');
		...
		await writeFile(path.join(unexpected, 'operon-cli-1.1.1.tgz'), fixture.tarball);

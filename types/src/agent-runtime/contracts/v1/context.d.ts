import type { MutationKindV1 } from './capabilities.js';
import type { CatalogPoliciesV1, CatalogTaxonomyV1, FieldDescriptorV1, FieldValueTypeV1 } from './catalog.js';
import { AffectedResourceRevisionMapV1, ContextRevisionV1, EntityCandidateV1, SourceRevisionV1, TaskIdentityV1, TaskSelectorV1, TaskSourceLocatorV1 } from './identity.js';
import { ConsistencyV1, CONTRACT_VERSION_V1, ContractWarningV1, FreshnessV1, JsonValue, ProvenanceV1, StructuredErrorV1, TruncationV1 } from './primitives.js';
export type { EntityCandidateV1, TaskSelectorV1, } from './identity.js';
export declare const CONTEXT_PROJECTIONS_V1: readonly ["exact-task", "task-neighborhood", "project-analysis", "planning-workload", "creation-context", "mutation-preview", "placement-candidates"];
export type ContextProjectionV1 = typeof CONTEXT_PROJECTIONS_V1[number];
export declare const CONTEXT_PURPOSES_V1: readonly ["read", "analysis", "planning", "creation", "mutation-readiness"];
export type ContextPurposeV1 = typeof CONTEXT_PURPOSES_V1[number];
export declare const MUTATION_READINESS_OPERON_IDS_MIN_V1 = 2;
export declare const MUTATION_READINESS_OPERON_IDS_MAX_V1 = 64;
export interface ContextProjectionLimitV1 {
    defaultLimit: number;
    hardLimit: number;
    maxDepth: number | null;
}
export declare const CONTEXT_PROJECTION_LIMITS_V1: Readonly<Record<ContextProjectionV1, ContextProjectionLimitV1>>;
export declare const CONTEXT_HYDRATION_KEYS_V1: readonly ["notes", "links", "custom-fields", "source-markdown", "tracker-history", "reminder-items"];
export type ContextHydrationKeyV1 = typeof CONTEXT_HYDRATION_KEYS_V1[number];
export declare const TASK_GET_HYDRATION_KEYS_V1: readonly ["notes", "links", "custom-fields", "source-markdown", "tracker-history", "reminder-items", "writable-fields"];
export type TaskGetHydrationKeyV1 = typeof TASK_GET_HYDRATION_KEYS_V1[number];
export declare const CONTEXT_HYDRATION_CAPS_V1: Readonly<{
    noteBytesPerTask: number;
    linksPerTask: 50;
    customFieldsPerTask: 64;
    customFieldBytesPerTask: number;
    sourceMarkdownBytesPerTask: number;
    trackerHistoryItemsPerTask: 100;
    trackerHistoryBytesPerTask: number;
    reminderItemsPerTask: 256;
    reminderItemIdBytes: 256;
    reminderItemValueBytes: number;
    reminderItemsBytesPerTask: number;
    writableFieldsPerTask: 512;
    writableFieldKeyCharacters: 256;
    writableFieldValueBytes: number;
    writableFieldsBytesPerTask: number;
...
export declare const TASK_FINDER_SCOPES_V1: readonly ["normal", "overdue", "happens-today", "recent"];
export type TaskFinderScopeV1 = typeof TASK_FINDER_SCOPES_V1[number];
export declare const TASK_FINDER_REPRESENTATIONS_V1: readonly ["inline", "file"];
export type TaskFinderRepresentationV1 = typeof TASK_FINDER_REPRESENTATIONS_V1[number];
export declare const TASK_FINDER_PROJECT_MODES_V1: readonly ["direct", "tree"];
...
export declare const RELATIONSHIP_KINDS_V1: readonly ["parent", "child", "blocking", "blocked-by", "related", "ancestor", "project-member"];
...
export type PlacementCandidatesV1 = PlacementCandidatePageV1 & {
    mode: 'files';
...
export declare function resolveProjectionBoundsV1(projection: ContextProjectionV1, requestedLimit?: number, requestedDepth?: number): {
    limit: number;
    depth: number | null;
};

...
const PRIVATE_WORKFLOW_SHA256 = '3a063561b75318d8c3cb21fd57bff9249823e3dfb7ae11064989d4d0a09120ad';
...
async function createCandidate(npmRoot, outputRoot, runnerId) {
	...
	assert.equal(packed.version, '1.2.0');
	const source = path.join(outputRoot, packed.filename);
	const destination = path.join(outputRoot, 'operon-cli-1.2.0.tgz');
	if (source !== destination) await rename(source, destination);
...
async function compareCandidates(inputRoot, outputRoot) {
	...
	for (const directory of await readdir(inputRoot, { withFileTypes: true })) {
		...
		try {
			...
			const tarballPath = path.join(root, 'operon-cli-1.2.0.tgz');
	...
	await mkdir(outputRoot, { recursive: true });
	await cp(path.join(ubuntu.root, 'operon-cli-1.2.0.tgz'), path.join(outputRoot, 'operon-cli-1.2.0.tgz'));
